"""Training script for latent diffusion baseline (without CLIP).

Entry point for latent diffusion training. Loads config and runs training.
"""
...
from trainers.diffusion_trainer import LatentDiffusionTrainer
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train latent diffusion baseline (no CLIP)")
    
    # Config files
    parser.add_argument(
        "--train_config",
        type=str,
        default="configs/train/diffusion_train.yaml",
...
def create_diffusion_model(diffusion_config: dict, device: str) -> LatentDiffusionUNet:
    """Create diffusion U-Net model from config."""
    ...
    logger.info(f"Diffusion U-Net created:")
...
def create_datasets(train_config: dict, overfit_mode: bool = False, overfit_num: int = 16):
    ...
    rgb_transform = T.Compose([
        T.Resize((data_config['image_size'], data_config['image_size']), interpolation=T.InterpolationMode.BILINEAR),
        T.ToTensor(),
    ])
    
    def paired_transform(rgb, coarse_mask, refined_mask):
        """Apply transforms to paired data."""
        # Transform RGB (even though we don't use it yet)
        rgb = rgb_transform(rgb)
        # Transform masks
        coarse_mask = mask_transform(coarse_mask)
        refined_mask = mask_transform(refined_mask)
        return rgb, coarse_mask, refined_mask
    
    train_dataset = SurgicalMaskRefinementDataset(
        metadata_dir=data_config['metadata_dir'],
        split=data_config['split_train'],
        source=data_config['source'],
        load_images=True,  # Need to load images
        return_paths=False,
        apply_transforms=True,
        transform=paired_transform
    )
    
    val_dataset = SurgicalMaskRefinementDataset(
        metadata_dir=data_config['metadata_dir'],
...
def init_wandb(train_config: dict, diffusion_config: dict, vae_config: dict, run_name: str = None):
    ...
    # Initialize wandb
    project = wandb_config.get('project', 'surgical-latent-diffusion-baseline')
...
def main():
    ...
    print("=" * 70)
    print("LATENT DIFFUSION BASELINE TRAINING (NO CLIP)")
    print("=" * 70)
