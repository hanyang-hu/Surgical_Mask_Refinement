"""RGB-conditioned diffusion model trainer.

Handles training loop for RGB-conditioned latent diffusion model that uses:
- Frozen VAE for latent encoding/decoding
- Precomputed CLIP RGB tokens for conditioning
- Cross-attention mechanism for RGB conditioning
"""
...
class RGBConditionedLatentDiffusionTrainer:
    """Trainer for RGB-conditioned latent diffusion model.
    
    Manages diffusion training with CLIP RGB token conditioning including:
    - Frozen VAE latent encoding/decoding
    - Loading precomputed RGB tokens from dataset
    - RGB-conditioned noise prediction with cross-attention
    ...
    Args:
        model: RGBConditionedLatentDiffusionUNet model
        vae_interface: FrozenVAELatentInterface for encoding/decoding
        scheduler: LatentDiffusionScheduler for noise scheduling
        train_loader: Training dataloader (TokenConditionedMaskDataset)
        val_loader: Validation dataloader (TokenConditionedMaskDataset)
        optimizer: Optimizer
    ...
    def __init__(
        ...
        logger.info(f"RGBConditionedLatentDiffusionTrainer initialized")
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            rgb_tokens = batch['rgb_tokens'].to(self.device)        # [B, 196, 768]
            ...
            assert rgb_tokens.shape[1:] == (196, 768), f"Expected [B,196,768], got {rgb_tokens.shape}"
            ...
            # Predict noise with RGB-conditioned U-Net
            # Input: z_t, timesteps, z_coarse, rgb_tokens
            ...
            if not torch.isfinite(loss):
                ...
                logger.error(f"  rgb_tokens stats: min={rgb_tokens.min():.4f}, max={rgb_tokens.max():.4f}")
            ...
            if self.use_wandb and (self.global_step % self.log_every_n_steps == 0):
                wandb_metrics = {
                    ...
                    'train/rgb_tokens_std': rgb_tokens.std().item(),
                }
    ...
    def validate_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            rgb_tokens = batch['rgb_tokens'].to(self.device)
            ...
            if batch_idx == 0:
                vis_batch = {
                    ...
                    'rgb_tokens': rgb_tokens[:self.num_vis_samples],
                }
    ...
    def save_eval_visualization(self, vis_batch: Dict, epoch: int):
        ...
        Args:
            vis_batch: Dictionary with masks, latents, tokens, and predictions
        ...
        if self.use_wandb:
            ...
            wandb.log({
                'val/visualization': wandb.Image(
                    grid_np,
                    caption=f"Epoch {epoch}: Coarse GT | Refined GT | Decoded Coarse | RGB-Conditioned Pred"
                )
